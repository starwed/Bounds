            ...
            */
            step: function () {
                ...
                while (this.currentTime > nextGameTick) {
                    ...
                    dT = this.currentFrame - this.lastFrame;
                    if (dT > 20)
                        dT = 20
                    dT=20
                    Crafty.trigger("EnterFrame", { frame: frame++, t:dT });
                ...
                frameLag = this.renderTime - this.currentTime;
                
    
               if(this.currentTime > this.frameTime){
                    Crafty.trigger("MessureFPS",{value:this.frame, dT:dT});
                    this.frame = 0;
...
(function (parent) {
	...
	*/
		search: function (rect, filter) {
			...
			var i, j,k, hash, results = [];
			HashMap.key(rect, keys);
			...
			for (i = keys.x1; i <= keys.x2; i++) {
				//insert into all y buckets
				for (j = keys.y1; j <= keys.y2; j++) {
					var hMap = this.map[(i << 16)^j];

					if (hMap) {
                        for (k = 0; k<hMap.length; k++)
                            results.push(hMap[k]);
...
*/
	...
	*/
	HashMap.key = function (obj, keyholder) {
		if (obj.hasOwnProperty('mbr')) {
			obj = obj.mbr();
		}
		if (!keyholder)
			keyholder = {}
		keyholder.x1 = Math.floor(obj._x / cellsize),
		keyholder.y1 = Math.floor(obj._y / cellsize),
		keyholder.x2 =  Math.floor((obj._w + obj._x) / cellsize),
		keyholder.y2 =  Math.floor((obj._h + obj._y) / cellsize);
		return keyholder;
	};
